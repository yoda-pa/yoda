...
class TestKeybindings(TestCase):
    ...
    def runTest(self):
        ...
        result = self.runner.invoke(yoda.cli, ['keybindings','add','test_software',test_keybinding_filepath])
        ...
        result = self.runner.invoke(yoda.cli, ['keybindings','search','t-test_software','action1'])
        self.assertEqual(result.exit_code, 0)

        # Software Keybinding file exist
        result = self.runner.invoke(yoda.cli, ['keybindings','search','test_software','action1'])
        self.assertEqual(result.exit_code, 0)
