...
class TestDiary(TestCase):
    ...
    def runTest(self):
        ...
        self.assertEqual(result.exit_code, 0)
        result = self.runner.invoke(yoda.cli, ['diary', 'nt'], input="test task 3\n-")
        self.assertEqual(result.exit_code, 0)
        result=self.runner.invoke(yoda.cli, ['diary', 'ut'], input="1\nUpdated Task")
        self.assertEqual(result.exit_code, 0)
        result=self.runner.invoke(yoda.cli, ['diary', 'dct'], input="c")
        self.assertEqual(result.exit_code, 0)
        result=self.runner.invoke(yoda.cli, ['diary', 'dct'], input="e")
        self.assertEqual(result.exit_code, 0)
        ...
        self.assertEqual(result.exit_code, 0)
        result = self.runner.invoke(yoda.cli, ['diary', 'nn'], input="test\nnew note 1")
        self.assertEqual(result.exit_code, 0)
        result = self.runner.invoke(yoda.cli, ['diary', 'nn'], input="test2\nnew note 2")
        self.assertEqual(result.exit_code, 0)
