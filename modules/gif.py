...
@click.pass_context
def from_images(ctx):
    args = {ctx.args[i].strip('--'): ctx.args[i + 1] for i in range(0, len(ctx.args), 2)}
    try:
        source = args.pop('source')
    except KeyError:
        click.echo(chalk.red("Source parameter is missing. "
                             "Use --source to provide the path."))
        return
    try:
        output = args.pop('output')
    except KeyError:
        click.echo(chalk.red("Output path is missing. "
    ...
    try:
        ...
        click.echo(chalk.red('Could not write to file {}. '
